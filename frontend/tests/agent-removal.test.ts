...
const retired = [
  { source_id: 'agent-old', name: 'Garage Pi', retired_at: '2026-08-01T09:00:00Z', samples: 4210, oldest: '2026-05-02T06:00:00Z', newest: '2026-07-31T21:40:00Z' },
  { source_id: 'agent-blank', name: 'Bench unit', retired_at: '2026-08-20T12:00:00Z', samples: 0, oldest: null, newest: null },
]
...
function api(options: { retired?: unknown[] } = {}) {
  ...
  const fetchMock = vi.fn().mockImplementation((url: string, init?: RequestInit) => {
    ...
    if (url.includes('/agents/') && (init?.method ?? '') === 'DELETE') return Promise.resolve(new Response(null, { status: 204 }))
...
describe('removing an agent, and what it leaves behind', () => {
  ...
  it('accounts for what each retired source still holds', async () => {
    ...
    expect(rows[1]!.text()).toContain('Never reported')
  })
