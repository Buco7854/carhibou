          ...
          <div class="vehicle-card-body">
            ...
            <div class="vehicle-readings">
              <section class="charge-reading">
                <div><span><AppIcon name="battery" :size="15" />{{ t('vehicles.batteryLevel') }}</span><strong>{{ soc(vehicle) === null ? '—' : Math.round(soc(vehicle)!) }}<em>%</em></strong></div>
...
@media(max-width:560px){.garage-overview{grid-template-columns:1fr}.overview-heading{grid-column:auto}.overview-stat{grid-template-columns:34px 1fr;padding:13px 16px;border-right:0;border-bottom:1px solid var(--line)}.overview-stat:last-child{border-bottom:0}.stat-icon{width:34px;height:34px}.overview-stat div{display:flex;align-items:baseline;justify-content:space-between;gap:12px}.overview-stat strong{margin:0;font-size:20px}.roster-controls{flex-direction:column}.filter-tabs{display:grid;grid-template-columns:repeat(3,1fr)}.vehicle-visual{aspect-ratio:4/3}.vehicle-card-body{padding:18px 16px 0}.vehicle-readings{margin-top:22px}.charge-reading{max-width:none}.vehicle-readings dl{align-items:flex-start;flex-direction:column;gap:10px}.vehicle-card>footer{justify-content:stretch;padding:7px 16px 16px}.row-action{min-width:0;flex:1}.vehicle-identity{grid-template-columns:5px minmax(0,1fr)}.vehicle-identity>.status{grid-column:2;margin-top:3px}}
</style>
