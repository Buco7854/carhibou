...
<template>
  <div class="page vehicles-page">
    <header class="page-header">
      ...
      <div class="header-actions"><RouterLink class="button secondary" to="/profiles"><AppIcon name="profile" :size="15" />{{ t('profiles.title') }}</RouterLink><button class="button" @click="showForm = true"><AppIcon name="plus" :size="15" />{{ t('vehicles.add') }}</button></div>
    </header>

    <AppModal :open="showForm" :title="t('vehicles.create')" @close="showForm=false">
      <template #eyebrow><span class="eyebrow">{{ t('vehicles.add') }}</span></template>
      <form class="vehicle-create-form" @submit.prevent="create">
        <p class="vehicle-create-hint">{{ t('vehicles.createHint') }}</p>
        <label class="field"><span>{{ t('vehicles.name') }}</span><input v-model="form.name" class="input" required autofocus /></label>
        <p v-if="error" class="error" role="alert">{{ error }}</p>
        <div class="form-actions"><button class="button">{{ t('vehicles.create') }}</button><button class="button secondary" type="button" @click="showForm=false">{{ t('common.cancel') }}</button></div>
      </form>
    </AppModal>

    <AppModal :open="Boolean(deleteTarget)" :title="t('vehicles.deleteTitle')" @close="closeDelete">
      <template #eyebrow><span class="eyebrow">{{ t('vehicles.deleteEyebrow') }}</span></template>
      <div v-if="deleteTarget" class="vehicle-delete-confirmation">
        <div class="delete-warning-icon"><AppIcon name="trash" :size="20" /></div>
        <div><h3>{{ t('vehicles.deleteQuestion', { name: deleteTarget.name }) }}</h3><p>{{ t('vehicles.deleteWarning') }}</p></div>
      </div>
      <div class="form-actions delete-actions"><button class="button danger" type="button" :disabled="deleteBusy" @click="deleteVehicle"><AppIcon name="trash" :size="15" />{{ deleteBusy ? t('vehicles.deleting') : t('vehicles.deleteVehicle') }}</button><button class="button secondary" type="button" :disabled="deleteBusy" @click="closeDelete">{{ t('common.cancel') }}</button></div>
    </AppModal>
        ...
        <article v-for="vehicle in filteredVehicles" :key="vehicle.id" class="vehicle-card panel" :style="{ '--vehicle-color': vehicle.color || '#315fcf' }">
          ...
          <div class="vehicle-card-body">
            <header class="vehicle-identity">
              <div><h3>{{ vehicle.name }}</h3><p v-if="[vehicle.manufacturer, vehicle.model, vehicle.year].filter(Boolean).length">{{ [vehicle.manufacturer, vehicle.model, vehicle.year].filter(Boolean).join(' · ') }}</p></div>
            ...
            <div class="vehicle-profile"><AppIcon name="settings" :size="14" /><span>{{ t('vehicles.profile') }}</span><AppSelect class="card-profile-select" compact :model-value="vehicle.vehicle_profile" :aria-label="t('vehicles.profile')" @update:model-value="assignVehicleProfile(vehicle,$event)"><option :value="null">{{ t('vehicles.noProfile') }}</option><option v-for="profile in profiles" :key="profile.id" :value="profile.id">{{ profileNames[profile.id] }}</option></AppSelect></div>
          </div>

          <footer><button class="row-action danger" type="button" @click="deleteTarget=vehicle"><AppIcon name="trash" :size="15" />{{ t('common.delete') }}</button><RouterLink class="row-action" :to="`/vehicles/${vehicle.id}/history`"><AppIcon name="history" :size="15" />{{ t('vehicles.history') }}</RouterLink><RouterLink class="row-action primary" to="/devices"><AppIcon name="devices" :size="15" />{{ t('vehicles.tracker') }}</RouterLink></footer>
        </article>
...
.header-actions{display:flex;align-items:center;gap:9px}.field-hint{color:var(--muted);font-size:9px;line-height:1.45}.vehicle-create-form{display:grid;gap:17px}.vehicle-create-hint{margin:0;color:var(--muted);font-size:10px;line-height:1.55}.vehicle-create-form .form-actions{justify-content:flex-end;margin-top:2px}.vehicle-list{align-items:stretch}.vehicle-visual{min-width:0;min-height:0;flex:none;overflow:hidden;contain:size layout paint}.vehicle-visual :deep(.vehicle-media),.vehicle-visual :deep(.vehicle-media>img),.vehicle-visual :deep(.vehicle-photo-placeholder){width:100%;height:100%}.vehicle-profile{overflow:visible}.card-profile-select{min-width:150px;max-width:230px;margin-left:auto}.card-profile-select :deep(.app-select-trigger){min-height:30px;padding-block:5px;font-size:8px}.vehicle-card>footer .danger{margin-right:auto;color:var(--danger);border-color:color-mix(in srgb,var(--danger) 34%,var(--line))}.vehicle-card>footer .danger:hover{background:color-mix(in srgb,var(--danger) 8%,transparent);border-color:var(--danger)}.vehicle-delete-confirmation{display:grid;grid-template-columns:42px minmax(0,1fr);align-items:start;gap:14px}.delete-warning-icon{width:42px;height:42px;display:grid;place-items:center;color:var(--danger);background:color-mix(in srgb,var(--danger) 10%,var(--panel));border-radius:9px}.vehicle-delete-confirmation h3{margin:1px 0 6px;font-size:14px}.vehicle-delete-confirmation p{margin:0;color:var(--muted);font-size:10px;line-height:1.6}.delete-actions{justify-content:flex-end;margin-top:22px}.button.danger{color:#fff;background:var(--danger);border-color:var(--danger)}
...
@media(max-width:560px){.header-actions{width:100%;display:grid;grid-template-columns:1fr 1fr}.header-actions .button{width:100%}.garage-overview{grid-template-columns:1fr}.overview-heading{grid-column:auto}.overview-stat{grid-template-columns:34px 1fr;padding:13px 16px;border-right:0;border-bottom:1px solid var(--line)}.overview-stat:last-child{border-bottom:0}.stat-icon{width:34px;height:34px}.overview-stat div{display:flex;align-items:baseline;justify-content:space-between;gap:12px}.overview-stat strong{margin:0;font-size:20px}.roster-controls{flex-direction:column}.filter-tabs{display:grid;grid-template-columns:repeat(3,1fr)}.vehicle-visual{aspect-ratio:4/3}.vehicle-card-body{padding:18px 16px 0}.vehicle-readings{margin-top:22px}.charge-reading{max-width:none}.vehicle-readings dl{align-items:flex-start;flex-direction:column;gap:10px}.vehicle-card>footer{justify-content:stretch;padding:7px 16px 16px}.row-action{min-width:0;flex:1}.vehicle-identity{grid-template-columns:minmax(0,1fr)}.vehicle-identity>.status{grid-column:1;margin-top:3px}.vehicle-profile{align-items:flex-start;flex-wrap:wrap}.card-profile-select{width:100%;max-width:none;margin-left:0}}
.vehicle-card>footer button.row-action{cursor:pointer}
</style>
