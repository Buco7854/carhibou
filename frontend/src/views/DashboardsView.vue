...
<style scoped>
.dashboard-canvas{overflow:hidden;border-radius:6px}.canvas-heading{height:70px;display:flex;align-items:center;justify-content:space-between;padding:13px 17px;border-bottom:1px solid var(--line)}.canvas-heading h2{margin:0;font:600 21px "Barlow Condensed",sans-serif;text-transform:uppercase}.canvas-heading p{margin:4px 0 0;color:var(--muted);font:400 7px "IBM Plex Mono",monospace;text-transform:uppercase}.canvas-heading>span{padding:6px 8px;color:var(--muted);border:1px solid var(--line);font:400 7px "IBM Plex Mono",monospace;text-transform:uppercase}.grid-stack{padding:8px;background:color-mix(in srgb,var(--panel-2) 70%,var(--panel))}.grid-stack-item-content{inset:4px!important;border-radius:4px}.widget-remove{position:absolute;right:8px;top:7px;z-index:600;width:25px;height:25px;color:var(--muted);background:var(--panel-2);border:1px solid var(--line);cursor:pointer}.widget-remove:hover{color:var(--danger);border-color:var(--danger)}.modal-backdrop{position:fixed;inset:0;z-index:2000;display:grid;place-items:center;padding:20px;background:rgba(10,20,17,.62);backdrop-filter:blur(5px)}.modal{width:min(100%,470px);display:grid;gap:17px;padding:24px;border-radius:5px}.modal h2{font-family:"Barlow Condensed",sans-serif;text-transform:uppercase}
</style>
