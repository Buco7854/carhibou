...
<template>
  <main class="login-page">
    <section class="login-story">
      <div class="brand large"><span class="brand-mark">V</span><span>{{ APP_NAME }}</span></div>
      <div>
...
.login-story { position:relative; overflow:hidden; padding:50px clamp(35px,6vw,90px); display:flex; flex-direction:column; justify-content:space-between; background:linear-gradient(135deg,#0a2119,#07110e); border-right:1px solid var(--line); }
.brand.large { padding:0; font-size:24px; }.brand.large .brand-mark { width:40px;height:40px; }
.login-story h1 { font-size:clamp(38px,5.4vw,74px); line-height:1.04; letter-spacing:-.06em; margin:14px 0 20px; max-width:850px; }.login-story h1 em { color:var(--accent); font-style:normal; }.login-story p { max-width:590px; color:#9ab3a9; font-size:17px; line-height:1.7; }
.signal-grid { position:absolute; right:-80px; bottom:-80px; width:440px; height:440px; display:grid; grid-template-columns:repeat(6,1fr); opacity:.25; transform:rotate(-12deg); }.signal-grid i { border:1px solid #4b9677; border-radius:50%; margin:10px; animation:pulse 3s infinite alternate; }.signal-grid i:nth-child(3n) { animation-delay:1s; }
