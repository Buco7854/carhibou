...
const definitions: DashboardWidgetDefinition[] = [
  ...
  { type:'position-map', titleKey:'dashboards.positionMap', component:PositionMapWidget, defaultSize:{w:6,h:4}, general:true, needsMetric:false, configSchema:{fields:['vehicle_id','title','time_range_days']}, isEmpty:(_widget, vehicle) => neverReported(vehicle) },
  { type:'time-series', titleKey:'dashboards.timeSeries', component:TimeSeriesWidget, defaultSize:{w:6,h:3}, general:false, needsMetric:true, configSchema:{fields:['vehicle_id','metric','title','unit','time_range_days']}, isEmpty:(widget, vehicle) => neverReported(vehicle) || !widget.metric },
