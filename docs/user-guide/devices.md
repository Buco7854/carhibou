...
Create an enrollment for the intended vehicle and agent name, and choose how often the
agent reads the vehicle and how often it uploads. Both are per agent, because
agents on one account are not alike.

A cadence is two pairs, not one: a vehicle is parked for most of the month, and paying
the driving rate for that is what makes a fast cadence unaffordable. The agent decides
...
The figure beside them is calculated from the agent's actual upload payload and the
number of signals its profile decodes, weighed by how many hours a day you say the
vehicle is driven, so a metered plan can be matched to a cadence before the agent is
installed rather than after the bill arrives. It assumes the agent is powered all
...
## How the agent knows it is parked
...
1. **A readiness signal the profile decodes.** A profile that maps a frame to
   ...
   vehicle reporting ignition off is parked whatever else the agent sees. Charging
...
level crossing or a drive-through does not slow the agent down. An agent that has just
started has no evidence of anything and begins parked.

**Settings** on the agent card changes both afterwards. Renaming an agent does not
disturb it; changing a cadence issues a new configuration version, which the agent
picks up at its next sync. To apply it immediately, on the agent run:
...
invalidates the old value. The agent card reveals that replacement only for the
