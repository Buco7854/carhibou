...
| C-Zero battery SOC (`0x374`) | Complete | Passing fixture | Byte offset **corrected** 2026-08-26 from a script proven against a physical C-Zero: charge is byte 1, not byte 0. A recording taken before the correction read 91%, which is plausible; whether the corrected offset still reads plausibly is the first thing to check after updating |
...
| C-Zero pack power sign (charging vs delivering) | Complete | Passing unit conversion test | **Confirmed** 2026-08-26 from a live recording: stationary at 91% charge, 327.2 V and −7.6 A giving −2.5 kW, with the connector at 14.4 V. That is an AC charge at the rate this vehicle takes, so negative is absorbing, which is the convention every VehiNode surface expects |
