...
def test_four_personas_enforce_the_complete_access_model(
    ...
    # Catalog visibility belongs to the human realm and is account-wide; it does
    # not grant any vehicle access by itself.
    for session, _csrf in sessions.values():
        catalog = session.get("/api/v1/agent-implementations")
        assert catalog.status_code == 200
        assert [entry["id"] for entry in catalog.json()] == ["carhibou.go", "custom"]
    ...
    enrollment = admin.post(
        ...
        headers=admin_headers,
        json={"implementation_id": "custom", "name": "V1 agent"},
    )
    ...
    for persona, expected_status in (("viewer", 403), ("stranger", 404)):
        ...
        assert (
            _request(
                ...
                "post",
                f"/api/v1/vehicles/{v1['id']}/enrollments",
                json={"implementation_id": "custom", "name": "Denied"},
            ).status_code
            == expected_status
        )
